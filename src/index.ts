...
const submitForm = async (db: any, songId: number, na: string, si: string) => {
    ...
    if (checkSinger !== -1) {
        ...
        else if (checkAlbum === -1 && checkAuthor === -1) singer = getDetail.substring(7, getDetail.indexOf('Thể loại'));
        else if (checkAlbum === -1) singer = getDetail.substring(7, getDetail.indexOf('Sáng tác'));
    ...
    if (checkAlbum !== -1) {
        ...
        else if (checkAuthor === -1) album = getDetail.substring(getDetail.indexOf('Album') + 5, getDetail.indexOf('Thể loại'));
        else album = getDetail.substring(getDetail.indexOf('Album') + 5, getDetail.indexOf('Sáng tác'));
    }

    if (checkAuthor !== -1) {
        if (checkCategory === -1) author = getDetail.substring(getDetail.indexOf('Sáng tác') + 8, getDetail.length);
        else author = getDetail.substring(getDetail.indexOf('Sáng tác') + 8, getDetail.indexOf('Thể loại'));
    }
...
(async () => {
    ...
    db.all("SELECT field1, field2, field3 FROM roomsong", async (err, rows) => {
        ...
        for (let i = pos; i < length; i++) {
            ...
            fs.writeFile('miss.txt', i.toString(), err => {
                if (err) {
                    console.error(err)
                    return
                }
